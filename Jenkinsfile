        ...
        withSlack channel: 'jenkins', {
            ...
            stage('Test EmarsysSDK') {
            	sh "cd $derivedDataPath/Build/Products ; zip -r Tests.zip Debug-iphoneos Tests_iphoneos12.0-arm64e.xctestrun"
    					sh "gcloud alpha firebase test ios run --test $derivedDataPath/Build/Products/Tests.zip --device model=iphonexsmax,version=12.1,locale=en_GB --device model=iphone6,version=11.4,locale=en_GB --device model=ipad5,version=12.0,locale=en_GB --quiet --project ems-mobile-sdk"
            }
